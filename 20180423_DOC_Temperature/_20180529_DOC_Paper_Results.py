...
validation_strt = 355000+40000#300000+25000
validation_fnsh = 480000#validation_strt + 30000#319800
moving_avg_degr = 1000 #300
...
testX0 = df.loc[998:55000,['EngSpd_Norm', 'Torque_Norm', 'CoTemp_Norm','ExhFlw_Norm', 'IntPrs_Norm', 'APPsen_Norm', 'ThrtDS_Norm','CmprUS_Norm', 'Regenr_Norm', 'EGRsen_Norm']]
testX1 = df.loc[325000:355000,['EngSpd_Norm', 'Torque_Norm', 'CoTemp_Norm','ExhFlw_Norm', 'IntPrs_Norm', 'APPsen_Norm', 'ThrtDS_Norm','CmprUS_Norm', 'Regenr_Norm', 'EGRsen_Norm']]
testX = pd.concat([testX0, testX1])
testX= np.reshape(np.array(testX),(testX.shape[0],1,testX.shape[1]))

testY0 = df.loc[998:55000,['DOC_DS_Norm', 'DOC_US_Norm']]
testY1 = df.loc[325000:355000,['DOC_DS_Norm', 'DOC_US_Norm']]
testY = pd.concat([testY0, testY1])
time = df.loc[1:((355000-325000)+(55000-996)),['Time']]
...
plt.plot(time, scaler00.inverse_transform(testPredict[:,1].reshape(-1,1)), '-r', label = 'Model DOC_DS_T')
#plt.plot(scaler00.inverse_transform(testPredict),'-r',label = 'Model DOC_DS_T')
plt.plot(time, scaler00.inverse_transform(testY.DOC_DS_Norm.reshape(-1,1)), '-b', label = 'Actual DOC_DS_T')    # Reshaped
...
lo = pd.read_csv(loading + "/" + loading + "_Ext.csv", index_col=False)
plt.plot(lo['loss'])
plt.plot(lo['val_loss'])
